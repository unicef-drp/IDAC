...
# PATHS ----
projectFolder <- file.path(Sys.getenv("USERPROFILE"), "OneDrive - UNICEF/Migration and Displacement/IDAC Working Documents/J. IDAC Publications/9 facts/Version_2026/output") #Output files
...
# PACKAGES ----
library(pacman)
p_load(circlize, dplyr, eurostat, forcats, ggplot2, kableExtra, scales, stringr, tidyr, openxlsx)
...
#ggforce #for geo_circle plot in fact 7
#sp # plot coordinates
#plotrix #draw.circle
...
unrwa <- read.csv(paste0("https://sdmx.data.unicef.org/ws/public/sdmxapi/rest/",
                         "data/GLOBAL_DATAFLOW/PSE+LBN+JOR+SYR.MG_UNRWA_RFGS_CNTRY_ASYLM._T/",
                         "all/?format=csv"))
...
# MAP SETTINGS ----
source(file.path("01_9facts/map_settings.R"))
...
# TODO 2026 update: verify against the new IDMC/UNRWA reports that the 70%/30% split still applies to 2023-2025 before running
...
fact4.idmc[fact4.idmc$year %in% c(2023,2024,2025) & fact4.idmc$ISO3Code == "PSE", 6:12] <- fact4.idmc[fact4.idmc$year %in% c(2023,2024,2025) & fact4.idmc$ISO3Code == "PSE", 6:12] * .3
...
x_regions <- c("Sub-Saharan Africa", "Northern Africa and Western Asia", "Central and Southern Asia",
              "Eastern and South-Eastern Asia", "Europe and Northern America",
              "Latin America and the Caribbean", "Oceania")

vertices$Region[vertices$subgroup %in% x_regions] <- paste0("X", vertices$subgroup[vertices$subgroup %in% x_regions])
